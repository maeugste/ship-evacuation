...
% build open_second matrix
for i=1:config.open_second_nr
    config.open_second(i)=config.(sprintf('open_second_%i', i));
end
    
% save the "all exits open" configuration
config.floor(config.floor_exit).img_exit_second = config.floor(config.floor_exit).img_exit;
config.floor(config.floor_exit).img_wall_second = config.floor(config.floor_exit).img_wall;

% replace the open_second exits in img_exit with a wall
for i=1:config.open_second_nr
    config.floor(config.floor_exit).img_wall(find(config.exit_nr == config.open_second(i))) = 1;
    config.floor(config.floor_exit).img_exit(find(config.exit_nr == config.open_second(i))) = 0;
end
